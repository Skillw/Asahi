...
fun getClasses(pack: String): Set<Class<*>> {
    ...
    // 第一个class类的集合
    val classes: MutableSet<Class<*>> = java.util.LinkedHashSet()
    // 是否循环迭代
    val recursive = true
    // 获取包的名字 并进行替换
...
private fun getByInterface(clazz: Class<*>, classesAll: Set<Class<*>>): Set<Class<*>> {
...
internal fun String.existClass(): Boolean {
    return kotlin.runCatching { Class.forName(this) }.isSuccess
}


internal fun String.findClass(): Class<*>? {
    var clazz: Class<*>? = null
    val path = this
    kotlin.runCatching {
        clazz = Class.forName(path)
    }.exceptionOrNull()?.run {
        println("Cannot find the class $path")
    }
    return clazz
}